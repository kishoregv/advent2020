...
func main() {
	...
	for _, in1 := range numbers {
		for _, in2 := range numbers {
			if in1+in2 < 2020 {
				pair, found := binarySearch(in1+in2, numbers)
				if found {
					fmt.Printf("found the pair \n, %d + %d + %d = %d\n %d * %d * %d = %d \n", in1, in2, pair, in1+in2+pair, in1, in2, pair, in1*in2*pair)
					return
				}
			}
		}
